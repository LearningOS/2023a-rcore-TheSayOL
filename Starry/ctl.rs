...
pub fn syscall_renameat2(
    ...
    // 暂时不处理有 flag 的情况
    if !RenameFlags::from_bits(flags as u32)
        .unwrap()
        .contains(RenameFlags::NONE)
    {
        return Err(SyscallError::EPERM);
    }
    ...
    if !path_exists(old_path.path()) {
        error!("old path not exsit!");
        return Err(SyscallError::EPERM);
    }
    // new path 不存在, 直接替换
    if !path_exists(new_path.path()) {
        if let Err(e) = rename(old_path.path(), new_path.path()) {
            error!("newpath doesn't exsit, but rename failed");
            return Err(e.into());
        }
        return Ok(0);
    }

    // 如果 new path 已存在, 必须保证原子替换
    // 不会
    if path_exists(new_path.path()) {
        // 如果新旧都是同一个文件的硬链接, do nothing
        // 不会
        // 如果操作失败了, 必须保证在原地保留一个 newpath 的实例
        // 不会

        // 如果 old 是符号链接, 就 rename
        if metadata(old_path.path()).unwrap().file_type().is_symlink() {
            unimplemented!();
        }
        // 如果 new 是符号链接, 直接覆盖 new
        if metadata(new_path.path()).unwrap().file_type().is_symlink() {
            unimplemented!();
        }
        // 如果 old 是文件夹, 那么新必须是个空文件夹
        if metadata(old_path.path()).unwrap().is_dir() {
            if !metadata(new_path.path()).unwrap().is_dir() {
                return Err(SyscallError::EPERM);
            }
            // 需要判断 new 是否 empty, 不会
            unimplemented!();
            // old 是文件 new 是文件夹, 塞进去
        } else if metadata(new_path.path()).unwrap().is_dir() {
            let mut new = new_path.path().to_string();
            if !new_path.path().ends_with('/') {
                new = new + "/";
            }
            new += old_path.file().unwrap();
            if rename(old_path.path(), new.as_str()).is_err() {
                error!("rename old(file) into new(dir) failed");
                return Err(SyscallError::EPERM);
            }
            return Ok(0);
        ...
        // 如果都是文件 
            ...
            if remove_file(new_path.path()).is_err() {
                error!("remove new path failed");
                return Err(SyscallError::EPERM);
            }
            if rename(old_path.path(), new_path.path()).is_err() {
                error!("rename failed");
                return Err(SyscallError::EPERM);
            }
            return Ok(0);
    ...
    error!("renameat2: unknon problem");
    return Err(SyscallError::EPERM);
}
